...
{
DNAMeth<- readRDS(file.path(root, "dataset/AU_ICGC_DNAmeth.rds"))
#DNAMethb=DNAMeth
...
#save(met, file="AUDNAmeth.RData")
load("AUDNAmeth.RData")
load("normet.RData")
normet=normet[, !(colnames(normet)  %in% c("DO50272N"))]
...
#mval = t(apply(bval, 1,  function(x)asin(2*x - 1)))

#Mvalnormdf= t(apply(Normdf, 1,  function(x)asin(2*x - 1))

#########################START HERE############################################
#save(mval, file="pureMvalau.RData")

#save(bval, file="pureBvalau.RData")

#save(Normdf, file="norBvalau.RData")

#Mvalnormdf= t(apply(Normdf, 1, function(x) asin(x)))

#Mvalnormdf= t(apply(Normdf, 1,  function(x)asin(2*x - 1))

#save(Mvalnormdf, file="norMvalau.RData")


load("pureBvalau.RData")
load("norBvalau.RData")
mval= t(apply(bval, 1,  function(x)asin(2*x - 1)))
Mvalnormdf= t(apply(Normdf, 1,  function(x)asin(2*x - 1)))   
meth= merge(mval,Mvalnormdf, by=0)


load("/mmfs1/home/david.adeleke/WorkFlow/Chapter_Two/validation/dataset/auidtable.RData")

normid = colnames(Mvalnormdf)


tumtable=auidtable[auidtable$id  %in% colnames(mval),]
normtable=auidtable[1:ncol(Mvalnormdf),]
normtable$id=colnames(Mvalnormdf)
normtable$grp="WT"
...
sub.id=intersect(Clinical$id, sub.id)

#sub.id=setdiff(sub.id,excl)

meth=meth[,sub.id]
Clinical=Clinical[Clinical$id %in% sub.id,]
setequal(colnames(meth), Clinical$id)
...
sub.id= intersect(colnames(meth), Clinical$id)
meth=meth[,sub.id]
...
save(DP, file="DMethDE030324.RData")

save(meth, file= "meth030324.RData")
...
puree= as.data.frame(puree)
auidpuree= merge(puree, auidtable, by=0)

colnames(auidpuree)[2]= "purity"
auidpuree=auidpuree[auidpuree$purity >= 0.3,]

met= merge(normet, met, by=0)
...
normtable= auidpuree[1:length(normid),]
normtable$chemo=""
normtable$id=normid
normtable$grp="WT"
normtable$time=NA
...
auidpuree=as.data.frame(auidpuree)

puree=auidpuree[,c(1,2)]
...
commonid= intersect(auidpuree$id, colnames(met))
met= met [,commonid]

Norm.mat = met[,normid]

Tumor.mat = met[, !(colnames(met) %in% normid)]


Tumor.mat=as.matrix(Tumor.mat)
Tumor.mat= t(apply( Tumor.mat, 1, function(x) asin(x)))
...
purit= unique(purit)
rownames(purit)= purit$Row.names
ids= intersect(rownames(purit), colnames(Tumor.mat))

lamda = purit[ids, ]
lamda=na.omit(lamda)
lamda$purity=as.numeric(lamda$purity)
lamda=lamda[,-1]
...
meth=cbind(J,Norm.mat )

save(meth, file= "AUmethp.RData")

mval= meth[c("cg06952671" ,"cg10661002"),]

save(mval, file="AUDNAmval.RData")

#WorkingP

##############--------------------------#########################
meths= t(apply( meth, 1, function(x) sin(x)))
...
# load("/mmfs1/home/david.adeleke/adjustBetas-master/betaAdj.RData")

#excl= c("DO32748", "DO32808", "DO32757", "DO46627", "DO46659")


tumtable=usidtable[!(usidtable$icgc_donor_id %in% normid),]

normtable=usidtable[1:length(normid),]
normtable$icgc_donor_id=normid
