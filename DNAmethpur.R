...
#usidtable <- readRDS(file.path(root, idtable_files[coho]))
...
load("/mmfs1/home/david.adeleke/WorkFlow/Chapter_Two/validation/pureBval.RData")
load("/mmfs1/home/david.adeleke/WorkFlow/Chapter_Two/validation/norBval.RData")
...
tumtable=usidtable[usidtable$icgc_donor_id  %in% colnames(mval),]
...
sub.id= intersect(colnames(meth), Clinical$icgc_donor_id)
meth=meth[,sub.id]
