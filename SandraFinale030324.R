##############################TEST####################################
cpint= c("cg27106909", "cg20655792","cg03355101", 
         "cg06952671" , "cg21511203"  ,"cg14498666", "cg04147593",
         "cg10654546", "cg09200260")

cpint=unique(cpint)
cgint= c("MFNG", "FGFBP2", "CHMP4C", "ASPHD1", "TLX1", "C1orf210", "MPZL2", "HBG1"
         ,"MGC23284", "KAZ", "RSPH1", "SLC39A7", "FGFBP2" ,"PCDH1", "NEBL")

cgint=unique(cgint)

load("validation/dataset/auidtable.RData")

load("validation/adj.counts.au.RData")

load("pureMvalau.RData")



seq= adj.counts.au[cgint,];rm(adj.counts.au)
prob = mval[cpint,];rm(mval)
prob = t(prob)
seq = t(seq)
rownames(auidtable)=auidtable$id

aumemclin= merge(prob, seq, by=0)
rownames(aumemclin)= aumemclin$Row.names
aumemclin=aumemclin[,-1]
aumemclin = merge(aumemclin,auidtable, by=0)
aumemclin=aumemclin[,-1]
aumemclin=aumemclin[!is.na(aumemclin$time),]
aumemclin=aumemclin[aumemclin$time >= 5,]
load("mem.clin.RData")
aumemclin= mem.clin 
aumemclin=aumemclin[aumemclin$time >= 5,]
x <- as.matrix(aumemclin[, cpint])
y <- Surv(aumemclin$time, aumemclin$status)
...
modelph <- coxph(formula_str, data = aumemclin[aumemclin$time >=5,])

summary(modelph)
#C1orf210 FGFBP2
#MFNG FGFBP2 CHMP4C ASPHD1 TLX1 MPZL2
#cg27106909 cg06952671 cg21511203 cg09200260
# cg06952671 cg21511203 cg04147593

foi= c("cg27106909", "cg06952671", "cg21511203", "cg09200260", "C1orf210", "FGFBP2")
...
valist= list(RNA, Meth, Y)
save(valist, file="valist.RData")
# 
# Xdata1=Xdata1[,(intersect(colnames(Xdata1), colnames(test1)))]
# Xdata2=Xdata2[,(intersect(colnames(Xdata2), colnames(test2)))]
# test1=valist[[1]] 
# test1=test1[,(intersect(colnames(Xdata1), colnames(test1)))]
# test2=valist[[2]] 
# test2= test2[, (intersect(colnames(Xdata2), colnames(test2)))]
# Xtestdata= list(test1,test2)
# Ytest= valist[[3]] 
#####################################################################
...
load( "validation/DMGs030324.RData")
...
de.genes=DMGs
de.genes=unique(de.genes[de.genes$contrast %in% c("KpTp - WT", "KnTn - WT") & 
        abs(de.genes$logFC) >= 0.5 & de.genes$adj.P.Val < 0.05 ,7])

DP=unique((DP[DP$contrast %in% c("KpTp - WT", "KnTn - WT")  & 
        abs(DP$logFC) >= 0.2 & DP$adj.P.Val < 0.01 ,7]))
...
#save(fit.cvsida, Xdata, Y, file = "KpTnKptp130.RData")
...
netobj=networkPlot(fit.cvsida,cutoff=0.6) 
...
rsd=data.frame()
#"mycvselpcca",
for(i in c("fit.cvsida" )){
  i="fit.cvsida"
  rsd0= VarImportancePlot(get(i))
  rsd1=rsd0[[1]][[1]]
  rsd1=rsd1[rsd1$`Absolute Loading` !=0,]
  rsd1$grp=  "Supervised"
  
  colnames(rsd1)= c("View", "VarName", "Loading", "Absolute", "Normalized", "algorithm")
  ...
  rsd2$grp= "Supervised"
  colnames(rsd2)= c("View", "VarName", "Loading", "Absolute", "Normalized", "algorithm")
...
myresult=(selpscca.pred(Xdata1, Xdata2, Y,fitselpCCA=mycvselpcca, 
family="binomial",showProgress=T))


newpred= predict(myresult, newdata=Xdata1, newdata2=Xdata2)
Y.pred= round(newpred$pred.mod)

PerformanceMetrics(Y.pred, Y-1, family ='binomial')


Y.pred= fit.cvsida$PredictedClass -1
Y.act = Y-1

PerformanceMetrics(Y.pred, Y.act, family ='binomial')


print(summary(myresult[["mod.fit"]]))
#X1_2         -0.14269    0.08488  -1.681  0.09274 . 
#X2_1         -2.35915    0.84357  -2.797  0.00516 **



for(i in c("mycvselpcca")){
  i="mycvselpcca"
  rsd0= VarImportancePlot(get(i))
  rsd1=rsd0[[1]]
  rsd1=rsd1[rsd1$`View 1 Canonical Correlation Vector`==2 &rsd1$`Normalized Relative Importance` >=0.5,]
  rsd1=unique(rsd1)
  rsd1$grp= "Unsupervised"
  colnames(rsd1)= c("View", "VarName", "Loading", "Absolute", "Normalized", "algorithm")
  rsd1$View = 12
  
  rsd2=rsd0[[2]]
  rsd2=rsd2[rsd2$`View 2 Canonical Vector`==1 & rsd2$`Normalized Relative Importance` >=0.3,]
  rsd2=unique(rsd2)
  rsd2$grp= "Unsupervised"
  colnames(rsd2)= c("View", "VarName", "Loading", "Absolute", "Normalized","algorithm")
  rsd2$View = 21
  
  rsdb=rbind(rsd1,rsd2)
}


rsd=rbind(rsdb, rsd)

#save(rsd, file = "rsd.RData")
...
mem1=RNAlogCPM[cid,unique(rsd[rsd$algorithm == 'Supervised' &
                                rsd$View==1,2])]
mem2=meth[cid,unique(rsd[rsd$algorithm == 'Supervised' &
                           rsd$View==2,2])]
...
mem.clin5=mem.clin[mem.clin$time > 5 & mem.clin$grp %in% c("KnTn" , "KpTp"),]
#MFNG FGFBP2 CHMP4C ASPHD1 for all grp
#TLX1 C1orf210 MPZL2 HBG1 for HOMo
#MGC23284 KAZ RSPH1 SLC39A7 FGFBP2 PCDH1 NEBL for KpTn and KnTn KnTnHB
...
formula_str <- as.formula(paste("Surv(time, status) ~", paste(topg, collapse = " + ")))
# Fit Cox proportional hazards model
modelph <- coxph(formula_str, data = mem.clin5[mem.clin5$grp %in% c(jwd, dustin),])

summary(modelph)

suppressWarnings(ggforest(modelph))

# c("MFNG", "FGFBP2", "ASPHD1", "TLX1", "MPZL2" , "HBG1")

############probes
mem.clin5=mem.clin[mem.clin$time > 5 & mem.clin$grp %in% c("KpTn","KnTn", "KnTnHB", "KpTp", "KnTp"),]
...
#cg03355101 cg06952671 cg21511203  for all grp
#"cg27106909", "cg15474367" , "cg20655792",
#"cg14498666", "cg04147593" for HOMo
#cg10654546, cg09200260 for KpTn and KnTn KnTnHB



cpint= c("cg27106909", "cg15474367" , "cg20655792","cg03355101", 
        "cg06952671" , "cg21511203"  ,"cg14498666", "cg04147593",
        "cg10654546", "cg09200260")

cgint= c("MFNG", "FGFBP2", "CHMP4C", "ASPHD1", "TLX1", "C1orf210", "MPZL2", "HBG1"
         ,"MGC23284", "KAZ", "RSPH1", "SLC39A7", "FGFBP2" ,"PCDH1", "NEBL", "MGST1")
...
topp = c("cg27106909", "cg15474367", "cg20655792", "cg14498666","cg20691507", "cg24676817" ,"cg11228682" )
...
topg = c("MGST1", "C1orf210", "CHMP4C", "HBG1")
...
# Create a formula dynamically
foi= c("cg27106909", "cg15474367", "cg20655792", 
       "cg24676817", "cg11228682", "MGST1", "C1orf210", "HBG1" )
...
library(factoextra)
#mem.clin=mem.clin[mem.clin$grp %in% c("KnTnHB", "KnTn", "KpTp"),]
df=mem.clin[,2:507]
df=mem.clin[mem.clin$grp %in% c(dustin, jwd),unique(foi)]
...
mem.clin.surv = mem.clin[mem.clin$grp %in% c(jwd, dustin),]
mem.clin.surv$subtype=cluster_assignments

mem.clin.surv$subtype= ifelse(mem.clin.surv$subtype == 1, "Group_A",
                         ifelse(mem.clin.surv$subtype == "2", "Group_B", "Group_C"))

mem.clin$grp =as.character(mem.clin$grp )
mem.clin$grp=ifelse(mem.clin$grp =="KnTnHB", "KnTn",  mem.clin$grp)

dtt= mem.clin[mem.clin$grp %in% c("KpTp", "KnTn"),]
#table(mem.clin$grp, mem.clin$subtype)

table(dtt$grp, dtt$subtype)


sfit <- survfit(Surv(time, status)~subtype, data=dtt)
...
# Identify selected genes based on the optimal lambda value
optimal_lambda <- lasso_model$lambda.min
selected_genes <- coef(cv, s = lambda.min)
selected_genes <- selected_genes[selected_genes[, 1] != 0, ]
