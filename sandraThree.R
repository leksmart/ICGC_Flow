...
de.genes=de.genes[(de.genes$contrast %in% c(contrs, "KpTn -KnTn") )& 
                    abs(de.genes$logFC) >= 1 & de.genes$adj.P.Val < 0.05 ,7]

de.genes= unique(de.genes)

#DP=DVP
DP=DP[DP$contrast %in% c(contrs,  "KpTn -KnTn")  & 
...
write.table(netdfvt, "net3.txt",   col.names=TRUE,
            sep="\t", row.names=FALSE, quote=FALSE)


write.table(netdfed, "net3v.txt",   col.names=TRUE,
            sep="\t", row.names=FALSE, quote=FALSE)
...
write.table(DD, "DEGP3.txt",   col.names=TRUE,
            sep="\t", row.names=FALSE, quote=FALSE)
...
rsd1=rsd1[rsd1$`Mean Loading`>=0.1,]
...
topg = rsd[rsd$View ==1,2]

topp = rsd[rsd$View ==2,2]


mem.traits.phen=RNAlogCPM[,topg]
...
ggplot(mem.traits.phen, aes(x = grp, y =get(topg[iii]), fill = grp)) +
  geom_violin(trim = FALSE) + 
  geom_boxplot(width = 0.07) + 
  theme(legend.position = "none")+
  xlab("Mutation-based Subtypes") + 
  ylab(paste0("Mean Gene Expression: ",  (topg[iii]), " gene"))+
...
mem.traits.phen=  mem.traits.phen[ mem.traits.phen$grp %in% c("KpTp", "KnTn"),]

mem.traits.phen$grp = ifelse(mem.traits.phen$grp == "KnTnHB", 
                               "KnTn",  mem.traits.phen$grp )
...
features= c(topg, topp)
features=features[c(1,3,6,11,14,25)]
goi= c("TMOD3", "SLC9A3","BTBD6","FASN" ,"LOC647121" ,"CGB7" , "TLX1" ,"cg06952671" ,"cg10661002")

df <- allmem.traits.phen[, c(features)]
rownames(df)=allmem.traits.phen$Row.names
...
ggplot(allmem.traits.phen, aes(x = subtype, y =1/sqrt(DRS^2), fill = subtype)) +
  geom_violin(trim = FALSE) + 
  geom_boxplot(width = 0.07) + 
  theme(legend.position = "none",
        panel.background = element_rect(fill = "#f4f4f4"))+
